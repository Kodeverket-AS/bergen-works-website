    ...
    >
      ...
      >
        <div className='relative w-full h-80  md:w-1/2 '>
          <Image src={modalImageSrc} alt={modalImageAlt} fill className='object-contain' draggable={false} />
        </div>
        <div className='w-full md:w-1/2 text-dark-text p-6 lg:p-0  lg:pr-16 flex flex-col'>
          ...
          <p className='mb-4 text-lg  md:text-xl font-semibold'>{modalParagraph1}</p>
          <p className='mb-4 lg:text-lg'>{modalParagraph2}</p>
          <div className='mt-4 '>
            {isVippsLink ? (
              <VippsButton link={modalLink} />
            ) : (
              <Link
                onClick={onClose}
                href={modalLink}
